...
$config = new Thecodebunny\SpApi\Configuration([
    ...
    "endpoint" => Thecodebunny\SpApi\Endpoint::NA,
]);
```
...
`$config` can then be passed into the constructor of any `Thecodebunny\SpApi\Api\*Api` class. See the `Example` section for a complete example.
...
* `lwaRefreshToken (string)`: The LWA refresh token of the SP API application to use to execute API requests. Required, unless you're only using the `Configuration` instance to call [grantless operations](https://github.com/amzn/selling-partner-api-docs/blob/main/guides/en-US/developer-guide/Thecodebunny\SpApiDeveloperGuide.md#grantless-operations).
...
* `endpoint (array)`: Required. An array containing a `url` key (the endpoint URL) and a `region` key (the AWS region). There are predefined constants for these arrays in [`lib/Endpoint.php`](https://github.com/jlevers/selling-partner-api/blob/main/lib/Endpoint.php): (`NA`, `EU`, `FE`, and `NA_SANDBOX`, `EU_SANDBOX`, and `FE_SANDBOX`. See [here](https://github.com/amzn/selling-partner-api-docs/blob/main/guides/en-US/developer-guide/Thecodebunny\SpApiDeveloperGuide.md#selling-partner-api-endpoints) for more details.
...
* `onUpdateCredentials (callable|Closure)`: A callback function to call when a new access token is generated. The function should accept a single argument of type [`Thecodebunny\SpApi\Credentials`](https://github.com/jlevers/selling-partner-api/blob/main/lib/Credentials.php).
...
$docToDownload = new Thecodebunny\SpApi\Document($reportDocumentInfo, $reportType);
...
$docToUpload = new Thecodebunny\SpApi\Document($feedDocumentInfo, $feedType);
...
$docToDownload = new Thecodebunny\SpApi\Document($feedResultDocument, $feedType);
...
```php
$buyer = new Thecodebunny\SpApi\Model\Service\Buyer([
...
``` php
$serviceJob = new Thecodebunny\SpApi\Model\Service\Buyer([
    // ...
    "buyer" => $buyer,
    // ...
]);
